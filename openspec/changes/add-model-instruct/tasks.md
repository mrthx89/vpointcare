# Tasks: Tambahkan Model Instruct untuk Desk Assistant dan Inbox WhatsApp
...
- [x] Buat helper method untuk memilih model assistant, contoh:
  ...
  ```
- [x] Gunakan helper ini di `callProvider()` untuk mengganti penggunaan `$settings->ModelAi` untuk Desk Assistant.
- [x] Pastikan untuk OpenAI dan provider lain menggunakan model yang benar.

## 6. Fix Desk Assistant UI Bugs
- [x] Buka `src/resources/views/filament/pages/desk-assistant.blade.php`.
...
- [x] Perbarui `useSuggestedReply()` di `DeskAssistant.php` untuk clear `$this->suggestedReplies` setelah dipilih:
  ```php
...
- [x] Test Desk Assistant: kirim pesan dan pastikan provider menerima Model Instruct.
...
- [x] Test suggested replies di Desk Assistant: klik salah satu, pastikan input terisi dan suggested replies menghilang.
- [x] Periksa UI Desk Assistant: pastikan tidak ada shadow pada area bawah dan textarea memiliki max-height yang wajar.
