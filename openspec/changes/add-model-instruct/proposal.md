# Change: Tambahkan Model Instruct untuk Desk Assistant dan Inbox WhatsApp

## Summary

Tambahkan kolom **ModelInstructAi** pada pengaturan AI Agent untuk memisahkan model yang dipakai untuk auto-reply customer (menggunakan Model Utama / `ModelAi` existing) dengan model yang dipakai untuk internal assistant seperti Desk Assistant dan opsi jawaban ringan WhatsApp (menggunakan `ModelInstructAi` baru).
...
- Memisahkan konfigurasi model utama (untuk auto-reply customer) dan model instruct (untuk Desk Assistant dan opsi jawaban ringan WhatsApp).
...
- Memperbaiki tiga bug UI di Desk Assistant:
...
- `src/app/Services/Ai/InternalChatbotService.php` - Pemilihan model untuk Desk Assistant.
- `src/resources/views/filament/pages/desk-assistant.blade.php` - Perbaikan bug UI.
...
6. Perbaiki bug UI di `desk-assistant.blade.php`.
...
- Desk Assistant memakai `ModelInstructAi` jika terisi, jika tidak fallback ke `ModelAi`.
- Auto-reply customer **tetap** memakai `ModelAi` dan tidak terpengaruh.
- Test koneksi AI tetap memakai `Model Utama`.
- Suggested replies di Desk Assistant menghilang setelah salah satu dipilih.
- Area input Desk Assistant tidak memiliki shadow dan memiliki max-height yang wajar.
