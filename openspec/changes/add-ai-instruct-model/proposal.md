...
- Mengarahkan Desk Assistant memakai `ModelInstructAi` bila terisi.
...
- Tidak mengubah format request provider selain nilai `model` untuk Desk Assistant dan Inbox WhatsApp.
...
- Test/validasi manual Desk Assistant, test koneksi AI, dan auto-reply.
...
- Pencarian kode saat ini menunjukkan opsi jawaban terstruktur (`suggested_replies`) ada di Desk Assistant; Inbox WhatsApp belum memiliki generator opsi jawaban ringan terpisah.
- `AiAutoReplyService` juga memakai model utama dari setting untuk flow auto-reply.
- `DeskAssistant` memiliki fallback settings array dengan `ModelAi => null` ketika setting DB tidak tersedia.
...
- Help text **Model Instruct**: dipakai Desk Assistant agar suggestion/draft lebih cepat.
...
- Jika `ModelInstructAi` terisi, Desk Assistant memakai nilai itu.
- Jika `ModelInstructAi` kosong, Desk Assistant memakai `ModelAi`.
...
- Opsi jawaban ringan di Desk Assistant dan Inbox WhatsApp memakai `ModelInstructAi` karena hasilnya berupa saran/draft yang direview user sebelum dikirim.
...
5. Update `InternalChatbotService` agar Desk Assistant memakai `ModelInstructAi` dengan fallback.
...
- Desk Assistant memakai `ModelInstructAi` saat field tersebut terisi.
...
- Desk Assistant fallback ke `ModelAi` saat `ModelInstructAi` kosong.
