...
Route::prefix('auth/{provider}')
    ->whereIn('provider', ['google', 'sso'])
    ->name('external-auth.')
    ->group(function (): void {
        Route::get('redirect', [ExternalAuthController::class, 'redirect'])->name('redirect');
        Route::get('callback', [ExternalAuthController::class, 'callback'])->name('callback');
    });
