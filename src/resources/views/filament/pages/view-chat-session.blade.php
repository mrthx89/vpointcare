<x-filament-panels::page>
    ...
    {{-- ── Header Info Sesi ── --}}
    <div class="rounded-xl border border-gray-200 bg-white px-5 py-3 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <div class="flex flex-wrap items-center gap-x-4 gap-y-2">

            {{-- Nama kontak & status --}}
            <div class="flex items-center gap-2 min-w-0">
                <span class="text-base font-bold text-gray-900 dark:text-white truncate">{{ $session['NamaKontak'] }}</span>
                <span class="shrink-0 rounded-full px-2 py-0.5 text-xs font-medium
                    {{ str_contains(strtolower($session['Status']), 'selesai') || str_contains(strtolower($session['Status']), 'tutup')
    ...
    <div class="mt-4 flex gap-4" style="height: calc(100vh - 260px); min-height: 400px;">
        ...
        <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            ...
            <div class="min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden bg-gray-50 p-4 dark:bg-gray-950/60">
                @forelse ($messages as $msg)
                    @php
                        ...
                        $isImage   = str_contains($jenis, 'image') || str_contains($jenis, 'gambar') || str_contains($jenis, 'photo') || str_contains($jenis, 'stiker');
                        $isVideo   = str_contains($jenis, 'video');
                        $isAudio   = str_contains($jenis, 'audio') || str_contains($jenis, 'voice') || str_contains($jenis, 'ptt');
                        $mediaUrl  = $msg['UrlMedia'] ? route('admin.waha-media.show', ['message' => $msg['Id']]) : null;
                            ...
                            <div class="mt-2 overflow-hidden rounded-md {{ $isOut ? 'bg-blue-700/40' : 'bg-gray-100 dark:bg-gray-950' }}">
                                @if ($mediaUrl && $isImage)
                                    <a href="{{ $mediaUrl }}" target="_blank" rel="noopener" class="block">
                                        <img src="{{ $mediaUrl }}" alt="{{ $mediaLabel }}" class="max-h-80 w-full object-contain">
                                    </a>
        ...
        <div class="flex w-72 shrink-0 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900" style="max-width: 320px;">
            ...
            <div class="min-h-0 flex-1 overflow-y-auto p-3 space-y-3">
                @forelse ($internalNotes as $note)
                    <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
                        <p class="text-sm text-gray-800 dark:text-gray-100 whitespace-pre-wrap leading-relaxed">{{ $note['IsiCatatan'] }}</p>
                        <div class="mt-2 flex items-center justify-between text-xs">
                            <span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $note['NamaPembuat'] }}</span>
