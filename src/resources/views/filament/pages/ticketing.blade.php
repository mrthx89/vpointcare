<x-filament-panels::page>
    <div class="space-y-6">
        <div class="grid gap-4 md:grid-cols-5">
            @foreach ([[__('ui.ticketing.new'), $stats['new']], [__('ui.ticketing.in_progress'), $stats['progress']], [__('ui.ticketing.overdue'), $stats['overdue']], [__('ui.ticketing.done'), $stats['done']], [__('ui.ticketing.my_tickets'), $myTickets]] as [$label, $value])
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <table class="min-w-full text-sm"><thead><tr class="border-b text-left"><th class="p-3">{{ __('ui.ticketing.number') }}</th><th class="p-3">{{ __('ui.ticketing.title') }}</th><th class="p-3">{{ __('ui.common.status') }}</th><th class="p-3">{{ __('ui.ticketing.pic') }}</th><th class="p-3">{{ __('ui.ticketing.target') }}</th></tr></thead>
                <tbody>@forelse($recentTickets as $ticket)<tr class="border-b border-gray-100 dark:border-gray-800"><td class="p-3 font-semibold">{{ $ticket->NomorTicket }}</td><td class="p-3">{{ $ticket->JudulTicket }}</td><td class="p-3">{{ $ticket->NamaStatusTicket ?: '-' }}</td><td class="p-3">{{ $ticket->NamaPengguna ?: '-' }}</td><td class="p-3">{{ $ticket->TglTargetSelesai ?: '-' }}</td></tr>@empty<tr><td colspan="5" class="p-6 text-center text-gray-500">{{ __('ui.ticketing.empty') }}</td></tr>@endforelse</tbody>
            </table>
        </div>
    </div>
</x-filament-panels::page>
