        ...
        <div class="grid min-h-[680px] gap-4 xl:grid-cols-[340px_minmax(0,1fr)_340px]">
            <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="max-h-[620px] divide-y divide-gray-100 overflow-y-auto dark:divide-gray-800">
                    @forelse ($chatRows as $chat)
                        ...
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="truncate text-sm font-semibold text-gray-950 dark:text-white">{{ $chat['NamaInstansi'] }}</div>
                                ...
                                @if ($chat['BelumDibaca'] > 0)
                                    <div class="shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-semibold text-amber-700">{{ $chat['BelumDibaca'] }}</div>
                                @endif
                            </div>
                            <div class="mt-2 line-clamp-2 text-sm text-gray-600 dark:text-gray-300">{{ $chat['PesanTerakhir'] }}</div>
                    ...
                    @empty
                        <div class="p-6 text-center text-sm text-gray-500">Belum ada chat. Kirim POST ke endpoint webhook WAHA untuk mulai mengisi inbox.</div>
                    @endforelse
                </div>
            </section>
            ...
            <section class="flex min-h-[620px] flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                @else
                    <div class="flex flex-1 items-center justify-center p-6 text-sm text-gray-500">Pilih chat untuk melihat percakapan.</div>
                @endif
            </section>
            ...
            <aside class="space-y-4">
                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    ...
                    @if ($selectedChat)
                        <dl class="mt-4 space-y-3 text-sm">
                            <div><dt class="text-gray-500">Klien</dt><dd class="font-medium text-gray-900 dark:text-white">{{ $selectedChat['NamaInstansi'] }}</dd></div>
                            <div><dt class="text-gray-500">Jenis Chat</dt><dd class="font-medium text-gray-900 dark:text-white">{{ $selectedChat['JenisChat'] }}</dd></div>
                            <div><dt class="text-gray-500">Kontak</dt><dd class="font-medium text-gray-900 dark:text-white">{{ $selectedChat['NamaKontak'] }}</dd></div>
                            <div><dt class="text-gray-500">Grup</dt><dd class="font-medium text-gray-900 dark:text-white">{{ $selectedChat['NamaGrupWhatsapp'] ?: '-' }}</dd></div>
                        </dl>
                    @else
                ...
                <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    ...
                    <code class="mt-2 block rounded-md bg-gray-100 p-3 text-xs text-gray-700 dark:bg-gray-950 dark:text-gray-300">POST /webhooks/waha/{token}</code>
                    <p class="mt-3 text-sm text-gray-600 dark:text-gray-300">Jika token dikosongkan di .env, endpoint menerima request tanpa token.</p>
                </div>
            </aside>
        </div>
    </div>
