<x-filament-panels::page>
    ...
    >
            ...
            <div class="pointer-events-auto flex w-full max-w-3xl justify-end">
                @if (! empty($messages))
                    <button
                        ...
                        class="inline-flex items-center gap-1.5 rounded-full border border-gray-300 bg-white px-3 py-1.5 text-[13px] font-medium text-gray-600 transition hover:border-danger-300 hover:bg-danger-50 hover:text-danger-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-danger-500/40 dark:hover:bg-danger-500/10 dark:hover:text-danger-300"
                    >
            ...
            <div class="mx-auto flex w-full max-w-3xl flex-col gap-4">
                @if (empty($messages))
                    <div class="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center text-gray-500 dark:text-gray-400">
                        <div class="mb-4 rounded-full border border-gray-300 bg-white p-4 text-primary-600 dark:border-gray-600 dark:bg-gray-900 dark:text-primary-300">
                ...
                @foreach ($messages as $index => $message)
                    <div class="flex {{ $message['role'] === 'user' ? 'justify-end' : 'justify-start' }}">
                        <div class="group relative max-w-[min(44rem,90%)] rounded-2xl px-4 py-3 text-[15px] {{ $message['role'] === 'user' ? 'rounded-br-md bg-primary-600 text-white' : 'rounded-bl-md border border-gray-300 bg-white text-gray-900 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100' }} {{ ! empty($message['error']) ? 'border-danger-300 bg-danger-50 text-danger-700 dark:border-danger-500/30 dark:bg-danger-500/10 dark:text-danger-300' : '' }}">
                            ...
                            @if ($message['role'] === 'assistant')
                                <div class="vpoint-ai-markdown pr-8 text-[15px] leading-7">
                                    {!! $this->renderMarkdown((string) $message['content']) !!}
                                </div>
                            @else
                                <div class="whitespace-pre-wrap text-[15px] leading-7">{{ $message['content'] }}</div>
                            @endif

                            @if (! empty($message['attachments']))
                                <div class="mt-3 flex flex-wrap gap-1 border-t border-white/20 pt-2 text-[13px] opacity-85 dark:border-gray-700">
                            ...
                            @if (! empty($message['knowledge']))
                                <div class="mt-3 border-t border-gray-200 pt-2 text-[13px] opacity-75 dark:border-gray-700">
                                    {{ __('ui.chatbot.knowledge_used', ['knowledge' => implode(', ', $message['knowledge'])]) }}
                                </div>
                            @endif

                            <div class="mt-2 flex items-center justify-end gap-2 text-[12px] opacity-60">
                ...
                @if ($isTyping)
                    <div class="flex justify-start">
                        <div class="rounded-2xl rounded-bl-md border border-gray-300 bg-white px-4 py-3 text-[15px] text-gray-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-400">
        ...
        <div class="pointer-events-none fixed inset-x-0 bottom-0 z-20 flex justify-center px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-6 sm:px-6 sm:pb-5">
            <div class="pointer-events-auto w-full max-w-3xl">
                <div class="mx-auto flex flex-col gap-2 rounded-2xl border border-gray-300 bg-white p-2 ring-1 ring-gray-950/5 dark:border-gray-600 dark:bg-gray-900 dark:ring-white/10">
                    @if (! empty($attachments))
                        <div class="flex flex-wrap gap-1 px-2 pt-1 text-[13px] text-gray-500 dark:text-gray-400">
                        ...
                        <div class="flex max-h-24 flex-wrap gap-1 overflow-y-auto px-2 pt-1">
                            @foreach ($suggestedReplies as $reply)
                                <button
                                    ...
                                    class="inline-flex items-center rounded-full border border-gray-300 bg-gray-50 px-3 py-1.5 text-left text-[13px] font-medium text-gray-700 transition hover:border-primary-300 hover:bg-primary-50 hover:text-primary-700 dark:border-gray-600 dark:bg-gray-800/70 dark:text-gray-200 dark:hover:border-primary-500/40 dark:hover:bg-primary-500/10 dark:hover:text-primary-200"
                                >
                                    {{ $reply }}
                                </button>
                            @endforeach
                    ...
                    <form wire:submit.prevent="sendMessage" class="flex w-full items-end gap-2">
                        ...
                        <div x-data="{ open: false }" class="relative shrink-0">
                            <button type="button" x-on:click="open = ! open" x-on:click.outside="open = false" class="inline-flex h-10 items-center gap-1 rounded-full px-3 text-[13px] font-medium text-gray-600 transition hover:bg-gray-100 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-white">
                            ...
                            <div x-cloak x-show="open" x-transition.origin.bottom.right class="absolute bottom-12 right-0 w-72 overflow-hidden rounded-2xl border border-gray-300 bg-white p-2 text-sm ring-1 ring-gray-950/5 dark:border-gray-600 dark:bg-gray-900 dark:ring-white/10">
                                <div class="px-2 pb-1 pt-1 text-[12px] font-semibold uppercase tracking-wide text-gray-400">{{ __('ui.chatbot.response_mode') }}</div>
                                <button type="button" wire:click="$set('responseMode', 'light')" x-on:click="open = false" class="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-800">
                                    <span><span class="font-medium">{{ __('ui.chatbot.mode_light') }}</span><span class="block text-[12px] text-gray-500">{{ __('ui.chatbot.mode_light_desc') }}</span></span>
                                ...
                                <button type="button" wire:click="$set('responseMode', 'fast')" x-on:click="open = false" class="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-800">
                                    <span><span class="font-medium">{{ __('ui.chatbot.mode_fast') }}</span><span class="block text-[12px] text-gray-500">{{ __('ui.chatbot.mode_fast_desc') }}</span></span>
                                    @if ($responseMode === 'fast') <x-heroicon-m-check class="h-5 w-5 text-success-500" /> @endif
                                </button>

                                <div class="my-2 border-t border-gray-100 dark:border-gray-800"></div>
                                <div class="px-2 pb-1 text-[12px] font-semibold uppercase tracking-wide text-gray-400">{{ __('ui.chatbot.knowledge_mode') }}</div>
                                <button type="button" wire:click="$set('knowledgeMode', 'all')" x-on:click="open = false" class="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-800">
                                    <span><span class="font-medium">{{ __('ui.chatbot.knowledge_all') }}</span><span class="block text-[12px] text-gray-500">{{ __('ui.chatbot.knowledge_all_desc') }}</span></span>
                    ...
                    <div wire:loading wire:target="sendMessage" class="px-3 text-center text-[13px] font-medium text-primary-500 dark:text-primary-300">
                        {{ __('ui.chatbot.typing') }}
                    </div>

                    <div class="px-3 pb-0.5 text-center text-[12px] text-gray-400 dark:text-gray-500">
