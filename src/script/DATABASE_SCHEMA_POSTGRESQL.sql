...
VALUES
('ADMI', 'Admin', 'Akses penuh aplikasi'),
...
VALUES (
    ...
    'Anda adalah AI Agent customer service VPoint Care. Jawab dalam Bahasa Indonesia yang sopan, singkat, jelas, dan jangan membuat janji teknis yang belum dipastikan. Jika masalah perlu ditangani manusia, arahkan bahwa tim customer service akan menindaklanjuti.',
    'Terima kasih sudah menghubungi VPoint Care. Saat ini kami berada di luar jam operasional. Pesan Bapak/Ibu sudah kami terima dan akan kami tindak lanjuti pada jam kerja berikutnya.',
    'Terima kasih sudah menghubungi VPoint Care. Hari ini kami sedang libur ({nama_hari_libur}). Pesan Bapak/Ibu tetap kami terima dan akan kami teruskan ke tim customer service. Silakan sampaikan detail kendalanya agar tim kami bisa menindaklanjuti pada hari kerja berikutnya, {tanggal_masuk_kerja}. Mohon maaf atas ketidaknyamanannya.',
    'Halo, terima kasih sudah menghubungi VPoint Care. Saya bantu catat terlebih dahulu ya. Silakan jelaskan kendala yang sedang dialami, nanti tim customer service kami akan melanjutkan penanganannya.',
...
CREATE TABLE IF NOT EXISTS MStatusTask (Id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, KodeStatusTask varchar(50) NOT NULL UNIQUE, NamaStatusTask varchar(100) NOT NULL, Urutan int NOT NULL DEFAULT 0, StatusFinal BOOLEAN NOT NULL DEFAULT FALSE, Warna varchar(30) NULL, NonAktif BOOLEAN NOT NULL DEFAULT FALSE, TglBuat TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, DibuatOleh UUID NULL, TglEdit TIMESTAMP NULL, DieditOleh UUID NULL);

IF OBJECT_ID('TTask', 'U') IS NULL
CREATE TABLE IF NOT EXISTS TTask (Id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, NomorTask varchar(50) NOT NULL UNIQUE, IdTicket UUID NULL REFERENCES TTicket(Id), IdChat UUID NULL REFERENCES TChat(Id), IdCustomer UUID NULL REFERENCES MCustomer(Id), IdInstansi UUID NULL REFERENCES MInstansi(Id), IdTugasInduk UUID NULL, IdKategoriTicket UUID NULL REFERENCES MKategoriTicket(Id), IdPrioritasTicket UUID NULL REFERENCES MPrioritasTicket(Id), IdStatusTask UUID NOT NULL REFERENCES MStatusTask(Id), JudulTask varchar(255) NOT NULL, DeskripsiTask TEXT NULL, DitugaskanKepada UUID NULL REFERENCES MPengguna(Id), TglDitugaskan TIMESTAMP NULL, TglTargetSelesai TIMESTAMP NULL, TglSelesai TIMESTAMP NULL, TglDitutup TIMESTAMP NULL, DitutupOleh UUID NULL, EstimasiMenit int NULL, TglBuat TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, DibuatOleh UUID NULL, TglEdit TIMESTAMP NULL, DieditOleh UUID NULL, CONSTRAINT FK_TTask_TTask_Induk FOREIGN KEY (IdTugasInduk) REFERENCES TTask(Id));

IF OBJECT_ID('TTaskDPenugasan', 'U') IS NULL
CREATE TABLE IF NOT EXISTS TTaskDPenugasan (Id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, IdTask UUID NOT NULL REFERENCES TTask(Id), DitugaskanDari UUID NULL REFERENCES MPengguna(Id), DitugaskanKepada UUID NOT NULL REFERENCES MPengguna(Id), AlasanPenugasan varchar(500) NULL, TglPenugasan TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, TglBuat TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, DibuatOleh UUID NULL, TglEdit TIMESTAMP NULL, DieditOleh UUID NULL);

IF OBJECT_ID('TTaskDChecklist', 'U') IS NULL
CREATE TABLE IF NOT EXISTS TTaskDChecklist (Id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, IdTask UUID NOT NULL REFERENCES TTask(Id), JudulItem varchar(500) NOT NULL, Selesai BOOLEAN NOT NULL DEFAULT FALSE, Urutan int NOT NULL DEFAULT 0, TglSelesai TIMESTAMP NULL, DiselesaikanOleh UUID NULL, TglBuat TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, DibuatOleh UUID NULL, TglEdit TIMESTAMP NULL, DieditOleh UUID NULL);

IF OBJECT_ID('TTaskDKomentar', 'U') IS NULL
CREATE TABLE IF NOT EXISTS TTaskDKomentar (Id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, IdTask UUID NOT NULL REFERENCES TTask(Id), IsiKomentar TEXT NOT NULL, TglKomentar TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, TglBuat TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, DibuatOleh UUID NULL, TglEdit TIMESTAMP NULL, DieditOleh UUID NULL);

IF OBJECT_ID('TTaskDLampiran', 'U') IS NULL
CREATE TABLE IF NOT EXISTS TTaskDLampiran (Id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY, IdTask UUID NOT NULL REFERENCES TTask(Id), NamaFile varchar(255) NOT NULL, PathFile varchar(1000) NOT NULL, TipeFile varchar(100) NULL, UkuranFile bigint NULL, TglBuat TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, DibuatOleh UUID NULL, TglEdit TIMESTAMP NULL, DieditOleh UUID NULL);

IF OBJECT_ID('notifications', 'U') IS NULL
CREATE TABLE IF NOT EXISTS notifications (id UUID NOT NULL PRIMARY KEY, type VARCHAR(255) NOT NULL, notifiable_type VARCHAR(255) NOT NULL, notifiable_id UUID NOT NULL, data TEXT NOT NULL, read_at TIMESTAMP NULL, created_at TIMESTAMP NULL, updated_at TIMESTAMP NULL);

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'notifications_notifiable_type_notifiable_id_index') CREATE INDEX IF NOT EXISTS notifications_notifiable_type_notifiable_id_index ON notifications (notifiable_type, notifiable_id);

IF OBJECT_ID('MNomorDokumen', 'U') IS NULL
CREATE TABLE IF NOT EXISTS MNomorDokumen (Kode varchar(32) NOT NULL PRIMARY KEY, Nilai int NOT NULL DEFAULT 0, TglEdit TIMESTAMP NULL);
