...
class TaskResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            TextInput::make('NomorTask')->disabled()->dehydrated(false), TextInput::make('JudulTask')->required()->maxLength(255)->columnSpanFull(), Textarea::make('DeskripsiTask')->rows(5)->columnSpanFull(), Select::make('IdStatusTask')->options(fn () => self::options('MStatusTask', 'NamaStatusTask'))->required()->searchable(), Select::make('IdTicket')->options(fn () => DB::table('TTicket')->orderByDesc('TglBuat')->limit(500)->pluck('NomorTicket', 'Id')->all())->searchable(), Select::make('IdChat')->options(fn () => DB::table('TChat')->orderByDesc('TglChatTerakhir')->limit(500)->pluck('NamaKontak', 'Id')->all())->searchable(), Select::make('IdCustomer')->options(fn () => self::options('MCustomer', 'NamaCustomer'))->searchable(), Select::make('IdKategoriTicket')->options(fn () => self::options('MKategoriTicket', 'NamaKategori'))->searchable(), Select::make('IdPrioritasTicket')->options(fn () => self::options('MPrioritasTicket', 'NamaPrioritas'))->searchable(), Select::make('DitugaskanKepada')->options(fn () => self::options('MPengguna', 'NamaPengguna'))->searchable(), DateTimePicker::make('TglTargetSelesai')->native(false), TextInput::make('EstimasiMenit')->numeric()->minValue(0),
            ...
            Repeater::make('attachments')->relationship()->label(__('ui.ticketing.attachments'))->schema([FileUpload::make('PathFile')->disk('attachments')->directory('tasks')->maxSize(3072)->acceptedFileTypes(['image/*', 'application/pdf', 'text/plain', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'])->storeFileNamesIn('NamaFile')->required(), Placeholder::make('download')->content(fn ($record) => $record ? new HtmlString('<a href="'.route('admin.attachments.tasks.download', $record->Id).'">'.e(__('ui.ticketing.download')).'</a>') : '')])->mutateRelationshipDataBeforeCreateUsing(fn (array $data) => self::attachmentMetadata($data))->columnSpanFull(),
        ]);
    }

    public static function table(Table $table): Table
    {
        return $table->columns([TextColumn::make('NomorTask')->searchable()->sortable(), TextColumn::make('JudulTask')->searchable()->limit(50), TextColumn::make('status')->state(fn ($r) => DB::table('MStatusTask')->where('Id', $r->IdStatusTask)->value('NamaStatusTask') ?: '-')->badge(), TextColumn::make('assignee')->label(__('ui.ticketing.assigned_to'))->state(fn ($r) => DB::table('MPengguna')->where('Id', $r->DitugaskanKepada)->value('NamaPengguna') ?: '-'), TextColumn::make('TglTargetSelesai')->dateTime()->color(fn ($r) => $r->TglTargetSelesai?->isPast() && ! DB::table('MStatusTask')->where('Id', $r->IdStatusTask)->value('StatusFinal') ? 'danger' : null)])->filters([SelectFilter::make('IdStatusTask')->options(fn () => self::options('MStatusTask', 'NamaStatusTask')), Filter::make('mine')->label(__('ui.ticketing.my_tasks'))->query(fn (Builder $q) => $q->where('DitugaskanKepada', Auth::id()))])->recordActions([EditAction::make()])->defaultSort('TglBuat', 'desc');
    }
