---
source_file: "src/composer.json"
type: "code"
community: "Color Picker Module"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Color_Picker_Module
---

# composer.json

## Connections
- [[$schema]] - `contains` [EXTRACTED]
- [[autoload]] - `contains` [EXTRACTED]
- [[autoload-dev]] - `contains` [EXTRACTED]
- [[config]] - `contains` [EXTRACTED]
- [[description]] - `contains` [EXTRACTED]
- [[extra]] - `contains` [EXTRACTED]
- [[keywords]] - `contains` [EXTRACTED]
- [[license]] - `contains` [EXTRACTED]
- [[minimum-stability]] - `contains` [EXTRACTED]
- [[name]] - `contains` [EXTRACTED]
- [[prefer-stable]] - `contains` [EXTRACTED]
- [[require]] - `contains` [EXTRACTED]
- [[require-dev]] - `contains` [EXTRACTED]
- [[scripts]] - `contains` [EXTRACTED]
- [[type]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Color_Picker_Module